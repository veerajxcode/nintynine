...
function nintynine_posted_on() {
    ...
    $posted_on = sprintf(
        esc_html_x( 'Posted on %s', 'post_date', 'nintynine' ),
...
function nintynine_posted_by() {
    $byline = sprintf(
        esc_html_x( ' by %s', 'post_author', 'nintynine' ),
...
function nintynine_the_excerpt( $trim_character_count = 0  ) {
...
function nintynine_excerpt_more( $more = '') {
    if( ! is_single() ) {
        $more = sprintf( '<a class="nintynine-read-more text-white" href="%1$s"><button class="mt-4 btn btn-info">%2$s</button></a>' ,
        get_permalink( get_the_ID() ),
        __( 'Read more', 'nintynine' )
        );
    }
...
function nintynine_pagination() {
    ...
    printf( '<nav class="nintynine-pagination clearfix">%s</nav>', wp_kses( paginate_links( $args ), $allowed_tags ) );
}
 ...
 */
function nintynine_the_post_pagination( $current_page_no, $posts_per_page, $article_query, $first_page_url, $last_page_url, bool $is_query_param_structure = true ) {
	...
	<div class="mt-0 md:mt-10 mb-10 lg:my-5 flex items-center justify-end posts-navigation">
		...
		if ( 1 !== $current_page_no && ! empty( $first_page_url ) ) {
			printf( '<a class="first-pagination-link btn border border-secondary mr-2" href="%1$s" title="first-pagination-link">%2$s</a>', esc_url( $first_page_url ), __( 'First', 'nintynine' ) );
		}
		...
		echo paginate_links( [
			...
			'total'     => $total_pages,
			'prev_text' => __( 'Prev', 'nintynine' ),
			'next_text' => __( 'Next', 'nintynine' ),
		] );

		// Last Page
		if ( $current_page_no < $total_pages && !empty( $last_page_url ) ) {

			printf( '<a class="last-pagination-link btn border border-secondary ml-2" href="%1$s" title="last-pagination-link">%2$s</a>', esc_url( $last_page_url ), __( 'Last', 'nintynine' ) );
		}
 ...
 */
function nintynine_is_uploaded_via_wp_admin( $gravatar_url ) {
 ...
 * @return bool
 */
function nintynine_has_gravatar( $user_email ) {

	$gravatar_url = get_avatar_url( $user_email );

	if ( nintynine_is_uploaded_via_wp_admin( $gravatar_url ) ) {
		return true;
	}
